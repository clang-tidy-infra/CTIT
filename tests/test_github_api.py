...
from pr_watcher.github_api import MAX_ATTEMPTS, GitHubClient, GitHubError
...
class TestGitHubClientRetries(unittest.TestCase):
    ...
    def _attempts(self, method: str, error: Exception) -> int:
        ...
        with (
            patch("pr_watcher.github_api.urllib.request.urlopen", fake_urlopen),
            patch("pr_watcher.github_api.time.sleep"),
        ):
    ...
    def test_reports_the_status_in_the_error(self):
        with patch("pr_watcher.github_api.urllib.request.urlopen") as urlopen:
